...
func TestRegisterDomainSchemaSupportedCompilerVersions(t *testing.T) {
	for _, version := range []string{"v0.17.1", "v0.18.0", buildinfo.DevVersion} {
		t.Run(version, func(t *testing.T) {
			NewRegistry().RegisterDomainSchema(&DomainSchema{Domain: "test", Generated: &GeneratedInfo{CompilerVersion: version}})
		})
	}
}
...
func TestRegisterDomainSchemaPanicsOnLowCompilerVersion(t *testing.T) {
	...
	defer func() {
		...
		if !strings.Contains(value.(error).Error(), "generated by skelc v0.17.0 is lower than Vine required skelc version") {
