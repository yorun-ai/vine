...
func TestPortalEntryRepoStoresAccessAndName(t *testing.T) {
	...
	entry := &core.PortalEntry{Scheme: "https", Host: "demo.local", Port: 8443}
	repo.Save(entry)
	...
	// Hub labels an entry by the access Portal serves.
	assert.Equal(t, "https:demo.local:8443", got.Name)
	...
	builtIn := &core.PortalEntry{Scheme: "https", Host: "demo.local", Port: 8443, BuiltIn: true}
	repo.Save(builtIn)
	got, ok = repo.GetByAccess("https", "demo.local", 8443)
	require.True(t, ok)
	assert.Equal(t, entry.Id, got.Id)
