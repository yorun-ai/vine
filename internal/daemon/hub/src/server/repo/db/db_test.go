...
func TestNoDBCreatesIsolatedDatabasesAndSchemas(t *testing.T) {
	...
	for range 2 {
		...
		config.EnsureSchema()
		(&model.PortalSiteDao{Dao: rdb.NewDao[*model.PortalSite](db)}).EnsureSchema()
		(&model.PortalEntryDao{Dao: rdb.NewDao[*model.PortalEntry](db)}).EnsureSchema()
		(&model.PortalRuleDao{Dao: rdb.NewDao[*model.PortalRule](db)}).EnsureSchema()
		(&model.PortalCertDao{Dao: rdb.NewDao[*model.PortalCert](db)}).EnsureSchema()
		(&model.MetadataDao{Dao: rdb.NewDao[*model.Metadata](db)}).EnsureSchema()
