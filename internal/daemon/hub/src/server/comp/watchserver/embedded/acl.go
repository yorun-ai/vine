...
func (s *Store) authenticate(username string, password string) (_UserRole, bool) {
	switch username {
	case hubwatch.HubUsername:
		return authenticatePassword(password, s.hubPassword, userRoleHub)
	case hubwatch.LinkUsername:
		return authenticatePassword(password, hubwatch.LinkPassword, userRoleLink)
	case hubwatch.PortalUsername:
		return authenticatePassword(password, hubwatch.PortalPassword, userRolePortal)
	default:
