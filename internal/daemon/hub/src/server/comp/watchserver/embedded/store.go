...
var embeddedWatchServerLogger = logger.New("daemon:hub:watchserver:embedded")
...
func (s *Store) Start() {
	if s.inproc {
		...
		go s.serve(listener)
		s.endpoint = hubwatch.WatchInprocEndpoint
		embeddedWatchServerLogger.Info("vine.hub watch server started", "mode", "inproc", "endpoint", s.endpoint)
		return
	}
	...
	if err := waitRedisReady(listener.Addr().String(), 2*time.Second, readyTLSConfig); err != nil {
		s.Stop()
		vpre.CheckNilError(err, "watch server start failed")
	}
	s.endpoint = redisEndpoint(listener.Addr().String())
	if s.identity.Enabled() {
		s.endpoint = "rediss" + strings.TrimPrefix(s.endpoint, "redis")
	}
	embeddedWatchServerLogger.Info("vine.hub watch server started", "addr", listener.Addr().String(), "endpoint", s.endpoint)
}
...
func (s *Store) serve(listener net.Listener) {
	...
	if err != nil && !errors.Is(err, net.ErrClosed) {
		embeddedWatchServerLogger.Error("vine.hub watch server failed", "error", err)
	}
}
...
func (s *Store) InitRevision() {
	s.mutex.Lock()
	defer s.mutex.Unlock()
	if _, ok := s.getLocked(hubwatch.RevisionKey, timeNow()); ok {
		return
	}
	s.items[hubwatch.RevisionKey] = _Item{value: strconv.FormatInt(timeNow().UnixMilli(), 10)}
}
...
func (s *Store) ApplyAndNotify(operations []hubwatch.NotifyOperation) {
	now := timeNow()
	events := make([]hubwatch.Event, 0, len(operations))
	...
	for _, operation := range operations {
		if operation.Delete {
			...
			events = append(events, hubwatch.Event{
				Kind: hubwatch.EventKindDelete,
				Key:  operation.Key,
			})
			continue
...
func (s *Store) publish(key string, event hubwatch.Event) {
...
func (s *Store) nextRevisionLocked() uint64 {
	item, _ := s.getItemLocked(hubwatch.RevisionKey, timeNow())
	...
	s.items[hubwatch.RevisionKey] = item
	return revision
}
