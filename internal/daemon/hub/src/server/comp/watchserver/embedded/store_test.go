...
func TestStoreUsesMTLSIdentityForRedisRole(t *testing.T) {
	...
	store.Set(hubwatch.RevisionKey, "42")

	linkClient := newMTLSRedisClient(store.ListenAddr(), hubwatch.LinkUsername, linkIdentity.ClientConfig(daemon.HubIdentity.SPIFFEPath()))
	t.Cleanup(func() { _ = linkClient.Close() })
	value, err := linkClient.Get(context.Background(), hubwatch.RevisionKey).Result()
	require.NoError(t, err)
	assert.Equal(t, "42", value)

	impersonatingClient := newMTLSRedisClient(store.ListenAddr(), hubwatch.LinkUsername, portalIdentity.ClientConfig(daemon.HubIdentity.SPIFFEPath()))
	t.Cleanup(func() { _ = impersonatingClient.Close() })
	_, err = impersonatingClient.Get(context.Background(), hubwatch.RevisionKey).Result()
	require.Error(t, err)
