...
func newNetworkTestServer(t *testing.T) (*Server, *redis.Client) {
	...
	client := redis.NewClient(&redis.Options{
		Addr:            WatchListenAddrForTest(t, server),
		Protocol:        2,
		DisableIdentity: true,
		Username:        hubwatch.HubUsername,
...
func TestWatchInprocClientCommands(t *testing.T) {
	server, client := newInprocTestServer(t)
	ctx := context.Background()

	assert.Equal(t, server, hubwatch.InprocServer())
...
func TestWatchInprocClientHandshakeAndMutationCommands(t *testing.T) {
...
func TestWatchInprocClientACL(t *testing.T) {
	...
	client := newInprocRoleTestClient(hubwatch.LinkUsername, hubwatch.LinkPassword)
	t.Cleanup(func() {
		_ = client.Close()
	})
...
func TestWatchInprocClientPubSub(t *testing.T) {
	_, client := newInprocTestServer(t)
...
func TestWatchInprocDialFailsAfterStop(t *testing.T) {
	...
	_, err := hubwatch.DialInproc(ctx, "tcp", hubwatch.WatchInprocEndpoint)

	assert.EqualError(t, err, "watch inproc server is not initialized")
	assert.Nil(t, hubwatch.InprocServer())
}

func TestWatchInprocDialFailsWhenServerMissing(t *testing.T) {
	oldServer := hubwatch.InprocServer()
	hubwatch.SetInprocServer(nil)
	t.Cleanup(func() {
		hubwatch.SetInprocServer(oldServer)
	})

	_, err := hubwatch.DialInproc(context.Background(), "tcp", hubwatch.WatchInprocEndpoint)

	assert.EqualError(t, err, "watch inproc server is not initialized")
}

func TestWatchInprocDialFailsForRemoteServer(t *testing.T) {
...
func TestWatchServerACL(t *testing.T) {
	...
	linkClient := newNetworkRoleTestClient(t, server, hubwatch.LinkUsername, hubwatch.LinkPassword)
	assertWatchValue(t, ctx, linkClient, "config:feature-a", "config")
	assertWatchValue(t, ctx, linkClient, "rpc:demo.Service:endpoint:demo.app:instance", "rpc")
	...
	for name, credentials := range map[string][2]string{
		"wrong hub password":     {hubwatch.HubUsername, "wrong"},
		"nonempty link password": {hubwatch.LinkUsername, "wrong"},
...
func TestWatchServerCommands(t *testing.T) {
...
func TestWatchServerHandshakeAndPing(t *testing.T) {
	_, client := newNetworkTestServer(t)
	ctx := context.Background()
...
func TestWatchServerExpiryRemovesKeyFromReadsAndScan(t *testing.T) {
	_, client := newNetworkTestServer(t)
	ctx := context.Background()
	now := time.Now()
...
func TestWatchServerScanPaginatesWithCursor(t *testing.T) {
	_, client := newNetworkTestServer(t)
	ctx := context.Background()
...
func TestWatchServerIncr(t *testing.T) {
	_, client := newNetworkTestServer(t)
	ctx := context.Background()
...
func TestWatchServerPUnsubscribeStopsPatternMessages(t *testing.T) {
	_, client := newNetworkTestServer(t)
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
...
func TestWatchSetAndNotifyUpdatesRevisionAndPublishes(t *testing.T) {
	...
	assert.Equal(t, strconv.FormatUint(baseRevision+1, 10), mustGet(t, server, hubwatch.RevisionKey))

	message, err := pubsub.ReceiveMessage(ctx)
	require.NoError(t, err)

	var got hubwatch.Event
	require.NoError(t, json.Unmarshal([]byte(message.Payload), &got))
	assert.Equal(t, hubwatch.Event{
...
func TestWatchDeleteAndNotifyIgnoresMissingKey(t *testing.T) {
	server := NewServerForTest()
	baseRevision := testRevision(t, server)

	server.DeleteAndNotify("config:missing")

	assert.Equal(t, strconv.FormatUint(baseRevision, 10), mustGet(t, server, hubwatch.RevisionKey))
}

func TestWatchDeleteAndNotifyUpdatesRevisionAndPublishes(t *testing.T) {
	server, client := newNetworkTestServer(t)
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
...
func TestWatchNotifyBatchUsesSingleRevision(t *testing.T) {
	server, client := newNetworkTestServer(t)
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	...
	events := make([]hubwatch.Event, 0, 3)
	for range 3 {
		message, err := pubsub.ReceiveMessage(ctx)
		require.NoError(t, err)
...
func TestWatchNotifyBatchRejectsReuseAfterNotify(t *testing.T) {
	server := NewServerForTest()
	batch := server.NotifyBatch()
	batch.Notify()
...
func TestWatchKeepEphemeralUpdatesTTL(t *testing.T) {
	server := NewServerForTest()
	ttl := 180 * time.Second
...
func TestWatchServerRejectsInvalidCommands(t *testing.T) {
	_, client := newNetworkTestServer(t)
	ctx := context.Background()
...
func TestWatchServerScanReturnsMatchingKeysOnce(t *testing.T) {
	server := NewServerForTest()
...
func testRevision(t *testing.T, server *Server) uint64 {
	...
	value := mustGet(t, server, hubwatch.RevisionKey)
...
func assertWatchValue(t *testing.T, ctx context.Context, client *redis.Client, key string, expected string) {
	t.Helper()
