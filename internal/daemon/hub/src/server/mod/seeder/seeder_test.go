...
func newTestSeederRepos(t *testing.T) (*repo.AppConfigRepo, *repo.PortalRuleRepo, *repo.PortalCertRepo, *repo.PortalSiteRepo, *repo.MetadataRepo, *watchserver.Server) {
	...
	(&model.AppConfigDao{Dao: rdb.NewDao[*model.AppConfig](gdb)}).EnsureSchema()
	(&model.PortalEntryDao{Dao: rdb.NewDao[*model.PortalEntry](gdb)}).EnsureSchema()
	(&model.PortalRuleDao{Dao: rdb.NewDao[*model.PortalRule](gdb)}).EnsureSchema()
	(&model.PortalCertDao{Dao: rdb.NewDao[*model.PortalCert](gdb)}).EnsureSchema()
	(&model.PortalSiteDao{Dao: rdb.NewDao[*model.PortalSite](gdb)}).EnsureSchema()
