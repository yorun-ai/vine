...
const (
	...
	// debugging separated deployments. In production, Hub Watch mTLS binds each
	...
	// caller authentication, so the Watch endpoint must remain on a trusted network.
	LinkPassword   = ""
	PortalPassword = ""
)

// SPIFFEPathForUsername returns the daemon identity bound to a Hub Watch ACL
// username.
