...
应用侧接入层，负责把本地应用注册到 Hub、同步配置与服务发现状态，并为本地与远端应用之间提供统一的 Rpc 转发入口。Link 以应用 sidecar 的形式部署：它可以与应用共享进程，也可以作为同一主机上的另一个进程或 container，预期情况下两者处于同一个部署信任边界。特殊部署仍可将 Link API 监听到非 loopback 地址，但 Link 会针对这条未认证的跨主机 h2c 路径输出警告，因为它不是预期拓扑。
